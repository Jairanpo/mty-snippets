...
class SequenceSplitter():
    ...
    def create_breakdown(self, shot_name, requiered_nodes):
        ...
        for node in requiered_nodes:
            ...
            if node_type == 'reference':
                ...
                asset_entity = self.ensure_breakdown_entity(shot_entity, file_path, namespace)
                if asset_entity:
            ...
            if node_type == 'transform':
                ...
                if children and cmds.nodeType(children[0]) == 'gpuCache':
                    ...
                    asset_entity = self.ensure_breakdown_entity(shot_entity, file_path, None)
                    if asset_entity:
                        assets.append(asset_entity)
    ...
    def ensure_shot_entity(self, shot_name):
        ...
        if not current :
            ...
            print "Found shot entity: %s" % shot_entity
    ...
    def get_publish_camera_path(self, shot_name):
        ...
        template_fields['version'] = self.get_next_maya_shot_publish_version(shot_name)
    ...
    def get_next_maya_shot_publish_version(self, shot_name):
        ...
        if not shot_name in self.latest_shot_version_by_shot_name:
            ...
            publish = self.shotgun.find_one('PublishedFile', filters, fields, order)
    ...
    def publish_scene(self, shot_name):
        ...
        work_file_path = self.get_publish_scene_path(shot_name, "maya_shot_work")
        publish_file_path = self.get_publish_scene_path(shot_name, "maya_shot_publish")
    ...
    def extract_shot(self, shot_name):
        ...
        requiered_nodes = cmds.getAttr('%s.assets' % shot_node).split(';')
        
        # collect info from shot node
        camera_shape = cmds.shot(shot_node, query=True, currentCamera=True)
        camera_reference = cmds.referenceQuery(camera_shape, referenceNode=True)
        ...
        for reference_node in top_reference_nodes:
            if reference_node not in requiered_nodes:
                reference_file_path = cmds.referenceQuery(reference_node, filename=True)
        ...
        self.export_camera(camera_node, camera_publish_path, first_frame, last_frame)
        self.publish_camera(shot_name, camera_publish_path)

        # delete animation keys outside shot range
        global_infinity_range = 999999
        animation_curves = cmds.ls(type=['animCurveTT', 'animCurveTL', 'animCurveTA', 'animCurveTU'])
        for curve in animation_curves:

            # insert new keyframes in the limit of the shot range to mantain 
            cmds.setKeyframe(curve, animated=True, insert=True, time=first_frame)
            cmds.setKeyframe(curve, animated=True, insert=True, time=last_frame)

            # then remove all keyframes outside of thjat range
            cmds.cutKey(curve, animation="objects", time=(-global_infinity_range, first_frame-1))
            cmds.cutKey(curve, animation="objects", time=(last_frame+1, global_infinity_range))
